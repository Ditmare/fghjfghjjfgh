...
def shannon_fano(veroyatn):
    ...
    # Присваиваем коды
    for i, (index, _) in enumerate(sorted_symbols[:-1]):  # Для всех, кроме последнего
        # Код для первого символа будет "1", для второго "01", для третьего "001" и так далее
        codes[index] = '0' * i + '1'

    # Для последнего символа взять код предпоследнего и заменить 1 на 0
    last_index = sorted_symbols[-1][0]  # Индекс последнего символа
    codes[last_index] = codes[sorted_symbols[-2][0]][:-1] + '0'  # Код предпоследнего + '0' вместо '1'
